...
def cross_validation(cfg):
    ...
    for fold, (train_indices, test_indices) in enumerate(kfold.split(data_map_list)):
        ...
        core_train_subset = NASHDataset(core_root_dir, train_list, 'train', file_suffix=feats_suffix)
        core_valid_subset = NASHDataset(core_root_dir, test_list, 'test', file_suffix=feats_suffix)
        external_test_subset = NASHDataset(external_root_dir, test_list, 'test', file_suffix=feats_suffix)
        train_loader = DataLoader(core_train_subset, batch_size=batch_size, shuffle=True,num_workers=64)
        valid_loader = DataLoader(core_valid_subset, batch_size=batch_size, shuffle=True,num_workers=16)
        test_loader = DataLoader(external_test_subset, batch_size=batch_size, shuffle=False, num_workers=16)
