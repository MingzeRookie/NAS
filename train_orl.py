...
def train(train_loader, milnet, criterion, optimizer, cfg):
    ...
    for batch, (bags, bag_label) in enumerate(train_loader, start=1):
        ...
        with torch.autograd.set_detect_anomaly(True):
            bag_prediction, _, _ = milnet(bags)
...
def test(test_loader, milnet, criterion, cfg):
    ...
    with torch.no_grad():
        for bags, bag_label in test_loader:
            ...
            bags = bags.cuda()
            bag_prediction, cutpoints, var_scaling = milnet(bags)
            bag_loss = criterion(bag_prediction.cpu(), bag_label.long())
...
def main(args):
    ...
    save_path = os.path.join(config.get("output", "runs"), time_stamp)
