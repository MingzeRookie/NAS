...
def cross_validation(cfg):
    ...
    soft_sigma = dataset_cfg.get('sigma', 1)
    ...
    for fold, (train_indices, test_indices) in enumerate(kfold.split(data_map_list)):
        ...
        train_subset = NASHDataset(root_dir, train_list, 'train', file_suffix=feats_suffix, num_classes=num_classes, soft_sigma=soft_sigma)
...
class NASHDataset(Dataset):
    def __init__(self, root_dir, d_list, frac='train', file_suffix='.csv', num_classes=4, soft_sigma=0.5):
        self.root = root_dir
        ...
        for bag_idx, label in tqdm(self.bags_list):
            ...
            if frac == 'train':
                ...
                bag_label = self.softlabel(bag_label, num_classes, soft_sigma)
            else:
                bag_label = torch.tensor(label, dtype=torch.float32)
    ...
    def softlabel(self, bag_label, num_class, sigma):
