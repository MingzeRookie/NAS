...
class SSLMetaArch(nn.Module):
    ...
    def get_params_groups(self):
        ...
        for name, m in self.student.items():
            # 检查是否为应用了LoRA的模型
            if hasattr(m, 'model') and hasattr(m.model, 'peft_config'):
                logger.info(f"模块 {name} 使用LoRA微调")
                ...
                if lora_params:
                    ...
                    logger.info(f"  - 可训练LoRA参数数量: {sum(p.numel() for p in lora_params):,}")
                else:
                    # 如果没有找到LoRA参数，则使用默认参数组
                    logger.warning(f"  - 未找到LoRA参数，使用默认参数组策略")
                    params_groups = self.get_maybe_fused_params_for_submodel(m)
            else:
                # 对于其他模型，使用原始参数组获取方法
                params_groups = self.get_maybe_fused_params_for_submodel(m)
            
            all_params_groups.extend(params_groups)
