...
class UNISSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        if self.do_dino:
            ...
            # 最后初始化center
            with torch.no_grad():
                initial_center = torch.randn(1, self.dino_out_dim)
                initial_center = F.normalize(initial_center, dim=1)
                self.dino_loss.center = initial_center.cuda()
                logger.info(f"DINO中心点已初始化，范数: {torch.norm(initial_center).item():.4f}")
    ...
    def forward_backward(self, images, teacher_temp, grad_accumulation_steps=4):
        ...
        def get_teacher_output():
            ...
            # 使用更低的动量值更新中心点
            momentum = 0.5  # 降低动量值，加速中心点更新
            self.dino_loss.center = momentum * self.dino_loss.center + (1-momentum) * new_center

            # 定期记录中心点信息
            if self.training_steps % 10 == 0:
                center_norm = torch.norm(self.dino_loss.center).item()
                logger.info(f"中心点范数: {center_norm:.6f}")
            ...
            if not torch.isnan(new_center).any() and not torch.isinf(new_center).any():
                ...
                if self.training_steps % 10 == 0:  # 更频繁地记录
                    center_norm = torch.norm(self.dino_loss.center).item()
                    center_diff = torch.norm(old_center - self.dino_loss.center).item()
                    logger.info(f"中心点范数: {center_norm:.6f}, 变化量: {center_diff:.6f}")
    ...
    def update_teacher(self, m):
        ...
        for teacher_key, student_key in zip(self.teacher.keys(), self.student.keys()):
            ...
            # 处理DDP包装的模型
            student_module_unwrapped = getattr(student_module, 'module', student_module)
            
            # 获取参数字典以便按名称匹配
            student_params_dict = dict(student_module_unwrapped.named_parameters())
            
            # 逐参数更新
            for name, teacher_param in teacher_module.named_parameters():
                if name in student_params_dict:
                    student_param = student_params_dict[name]
                    
                    # 检查参数有效性
                    if torch.isnan(student_param.data).any() or torch.isinf(student_param.data).any():
                        logger.warning(f"跳过更新含有NaN/Inf的参数: {name}")
                        continue
        ...
        # 记录更新情况
        if hasattr(self, 'training_steps') and self.training_steps % 100 == 0:
            logger.info(f"教师模型已更新，动量值: {m:.6f}")
